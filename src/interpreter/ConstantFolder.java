...
public class ConstantFolder extends delphiBaseVisitor<Object> {
    ...
    public Map<ParserRuleContext, Object> getFoldedValues() {
        return foldedValues;
    }
    ...
    public Object visitExpression(delphiParser.ExpressionContext ctx) {
        Object left = tryFold(ctx.simpleExpression(0));
        if (ctx.relationaloperator() != null) {
            Object right = tryFold(ctx.simpleExpression(1));
            if (left != null && right != null) {
                Object result = foldRelational(ctx.relationaloperator(), left, right);
                if (result != null) {
                    foldedValues.put(ctx, result);
                    String exprText = ctx.getText();
                    System.out.println(exprText + " => " + result + " [FOLDED]");
                    return result;
                }
            }
            return null;
        ...
        if (left != null) {
            ...
            if (hasOperator(ctx.simpleExpression(0))) {
                System.out.println(ctx.getText() + " => " + left + " [FOLDED]");
            }
        }
        return left;
    }

    private boolean hasOperator(delphiParser.SimpleExpressionContext ctx) {
        return ctx.additiveoperator() != null;
    }

    private Object tryFold(delphiParser.SimpleExpressionContext ctx) {
        Object result = visitSimpleExpression(ctx);
        return result;
    }
